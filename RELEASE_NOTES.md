...
- **统一产品执行策略为 local-first：Free Personal 本地核心能力在所需 runtime 安装完成后不依赖 BossAI 登录、BossAI Points 或云端模型。**
- 本地核心链路明确为 Qwen（文案）、CosyVoice2（配音）、MuseTalk（数字人）、可选 Whisper（转写）和 FFmpeg（字幕/混音/成片）；本地 runtime 失败或缺失时不得静默回退到云端模型。
- BossAI Gateway 与 BYOK 都是用户主动选择的增强路径，不是 Free Personal 本地核心工作流的依赖；云端处理必须由用户显式触发。
...
- 当前 `0.1.0` 的真实登录契约保持为手机号/邮箱 + 密码；注册额外使用验证码 challenge/code。Video Agent 不持久化密码/验证码，也不建立第二套账号权威。无密码/验证码登录只能在 BossAI OS + Headquarters Commerce 提供权威兼容契约后切换。
...
- 另有一个 local-first GA 收口项：已登录状态下若 Headquarters entitlement 临时不可达，Pro/Business/云端能力应 fail-closed，但本地 Free Personal 核心能力必须继续可用；该行为需在 GA 前用自动化测试固化。
...
- **The unified execution policy is local-first: once the required runtimes are installed, Free Personal local core features do not depend on BossAI sign-in, BossAI Points or a cloud model.**
- The local core chain is explicitly Qwen (writing), CosyVoice2 (voice), MuseTalk (talking avatar), optional Whisper (transcription) and FFmpeg (subtitles/mixing/final composition). Missing or failed local runtimes must never silently fall back to a cloud model.
- BossAI Gateway and BYOK are explicit opt-in enhancements rather than dependencies of the Free Personal local core; cloud processing must be triggered explicitly by the user.
...
- The real `0.1.0` sign-in contract remains phone/email + password; registration additionally uses a verification challenge/code. Video Agent does not persist password/code values or create a second account authority. Passwordless/code-based sign-in may replace this only after BossAI OS + Headquarters Commerce expose an authoritative compatible contract.
...
- One additional local-first GA closure remains: when an already signed-in installation temporarily loses Headquarters entitlement service, Pro/Business/cloud capabilities must fail closed while the Free Personal local core stays available; this behavior must be locked down with automated coverage before GA.
